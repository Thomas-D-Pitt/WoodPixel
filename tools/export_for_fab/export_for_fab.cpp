...
int main(int argc, char *argv[])
{
  ...
  try
  {
    ...
    try{
    	Serializable::deserialize(tree_table_layout, "table_dimensions_mm", table_dimensions_mm, path_table_layout.parent_path());//erroring
    }
    catch (...){
    	std::cout << "An error happened here using width/height 12in/7in = 304.8mm/177.8mm\n";
    	table_dimensions_mm.width = 304.8;
    	table_dimensions_mm.height = 177.8;
    ...
    CutSaver::add_sources_bezier_svg(svg_saver, path_out.parent_path(), textures, patches, material_panels); //erroring
