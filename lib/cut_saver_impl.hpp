...
void CutSaver::add_sources_bezier(std::vector<SaverT> &saver, const boost::filesystem::path &path, const std::vector<Texture> &textures, const std::vector<Patch> &patches, const std::vector<MaterialPanel> &material_panels)
{
  ...
  for (const Patch &p : patches)
  {
    ...
    //std::cout << tex.marker.markers_pix.size();
    //std::cout << panel->markers_table_mm.size() << "\n";
    //std::cout << panel->markers_table_mm[0] << "\n";
    //std::cout << tex.marker.markers_pix[0] << "\n";
    
    cv::Mat T_texture_table;
    
    try{
    	T_texture_table = AffineTransformation::fit(tex.marker.markers_pix, panel->markers_table_mm);
    }
    catch (...){
    	std::cout << "Error in cut_saver_impl.hpp, unable to apply Affine Fit, no fit has been applyed!";
    	cv::Point2f srcTri[3];
    		srcTri[0] = cv::Point2f( 0.0, 0.f );
    		srcTri[1] = cv::Point2f( 1.f, 0.f );
    		srcTri[2] = cv::Point2f( 0.f, 1.f );
    	cv::Point2f dstTri[3];
    		dstTri[0] = cv::Point2f( 0.f, 0.f );
    		dstTri[1] = cv::Point2f( 1.f, 0.f );
    		dstTri[2] = cv::Point2f( 0.f, 1.f );
    	T_texture_table = cv::getAffineTransform(srcTri, dstTri);
    }
